...
Generates a 5-channel ego-centric rotated mini-map (84×84) for RL training.
Channels: Self, Enemies, Food, Boundary, Enemy Velocity.

All rasterization uses numpy vectorized ops — no Python pixel loops.
"""
import math
import numpy as np
...
# Pre-compute pixel coordinate grids (constant, allocated once)
_YY, _XX = np.mgrid[0:MAP_SIZE, 0:MAP_SIZE]  # shape (84, 84) each


def _world_to_ego(wx, wy, head_x, head_y, cos_a, sin_a):
    """Convert world coords to ego-centric rotated pixel coords."""
    dx = wx - head_x
    dy = wy - head_y
...
def _stamp_circle(channel, cx, cy, radius, intensity):
    """Draw a filled circle using vectorized numpy — no Python loops."""
    r = max(1, int(radius + 0.5))
    # Bounding box clip for speed
...
def _stamp_line(channel, x0, y0, x1, y1, intensity, thickness=1):
    """Draw a line using vectorized numpy distance-to-segment."""
    dx, dy = x1 - x0, y1 - y0
...
def generate_observation(player, snakes, foods, food_grid, world_radius):
    """
    Generate 5-channel 84×84 ego-centric mini-map.
    
    Returns: np.ndarray shape (5, 84, 84) dtype float32, values in [0, 1]
    """
    ...
    # --- Channel 3: Boundary (fully vectorized) ---
    dist_from_center = math.hypot(hx, hy)
    if dist_from_center + VIEW_RADIUS > world_radius - 300:
        # Convert every pixel to world coords in one shot
        ...
        boundary = np.clip(1.0 - d_to_edge / 300.0, 0.0, 1.0)
        obs[3] = boundary
    
    # --- Channel 4: Enemy Velocity ---
    for snake in snakes:
        if snake is player or snake.dead:
            continue
        head_d = math.hypot(snake.head[0] - hx, snake.head[1] - hy)
